    ...
    final productLike = {
      ...
      'images': state.images.map(
        (image) => image.replaceAll(
          '${Enviroment.apiUrl}/files/product', 
          '')).toList()
    };
    try {
...
final productFormProvider = StateNotifierProvider.autoDispose.family<ProductFormNotifier, ProductFormState, Product>(
  (ref, product) {
  final creeateUpdateCallback = ref.watch(epiProductsProvider.notifier).createOrUpdateProduct;
  return ProductFormNotifier(
    product: product,
    onSubmitCallback: creeateUpdateCallback
  );
});